  ...
  <head>
	...
	<script type="text/javascript">
		$(function(){
			...
			**/
			$('#dg').datagrid({  
			    url:'card/petro/ajax_list_mainCard.htm', 
			    striped:true,
			    loadMsg:'亲，正在加载ing...',
			    idField:'id',
			    pagination:true,
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						if(obj){
							if(obj.state==0 || obj.state ==4){ 
								hczd_sys.window.edit('选择下载条件','card/petro/toDownload.htm?main_card='+obj.card_no,600,200);
							 }else{
					...
					handler: function(){
						...
						window.open('card/petro/console_info.htm?cardNo='+obj.card_no+'&dateStr='+obj.dateStr,'newwindow'+obj.card_no,'height=500,width=700,top=0,left=0,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no');
						}else{
							$.messager.alert('提示','请选择一项进行操作！');
						}
					}
					...
					iconCls: 'icon-ok',
					handler: function(){
						hczd_sys.window.edit('下载缓存数据','card/petro/list_cache_main_card_excel.htm',700,400);
					}
				},'-',{
					text:'帮助',
					iconCls: 'icon-help',
